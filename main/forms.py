...
class RegistrationForm(UserCreationForm):
    ...
    def clean_referred_by(self):
        data = self.cleaned_data['referred_by']
        if data is not None or data != "":
            if not User.objects.filter(email__iexact=self.cleaned_data['referred_by']):
                raise ValidationError(_('Please input correct referred email'))
        return data
